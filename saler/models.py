...
class SellerSlider(models.Model):
	...
	def image_tag(self):
		return mark_safe('<img src="/media/%s" width="450" height="100" />' % (self.image))
...
class Product(models.Model):
	...
	TAG_COLOR_CHOICES = (("primary",'primary'),("secondary",'secondary'),("success",'success'),("danger",'danger'),("warning",'warning'),("info",'info'))
	...
	TotalCal = models.IntegerField(blank=True, null=True)
	URL= models.CharField(max_length=600,null=True,blank=True)
	brand = models.CharField(max_length=100,null=True,blank=True)
	color = models.ForeignKey(Color, on_delete=models.CASCADE,null=True,blank=True)
	price = models.IntegerField(default=0)
	price_not = models.IntegerField(blank=True,null=True)
	tag_title = models.CharField(max_length=20, null=True, blank=True)
	tag_color = models.CharField(max_length=20,choices=TAG_COLOR_CHOICES, null=True, blank=True)
	desc = models.TextField(blank=True)
	specs = models.TextField(null=True, blank=True)
	gst = models.CharField(default='0',max_length=3,choices=GST_CHOICES)
	pub_date = models.DateField(auto_now=True)
	image1 = models.ImageField(upload_to='products/images', default="",null=True,blank=True)
	...
	is_Veg=models.BooleanField(default=0,blank=True)
	is_SugarFree=models.BooleanField(default=0,blank=True)
	is_HighProtein=models.BooleanField(default=0,blank=True)
	is_GluttentFree=models.BooleanField(default=0,blank=True)
	is_Sattvik=models.BooleanField(default=0,blank=True)
	is_Vegan=models.BooleanField(default=0,blank=True)
	def save(self, *args, **kwargs):
		super().save(*args, **kwargs)
...
class WholeSaleProduct(models.Model):
	...
	def image_tag(self):
		return mark_safe('<img src="/media/%s" width="80" height="100" />' % (self.image1))
...
class Orders(models.Model):
	STATUS_CHOICES = (("Accepted",'Accepted'),("Packed",'Packed'),("On The Way",'On The Way'),("Delivered",'Delivered'),("Cancel",'Cancel'))
	order_id = models.CharField(max_length=50,default='')
	saler = models.CharField(max_length=100,default='AttiTude@admin',)
	user = models.ForeignKey(User, default='', on_delete=models.CASCADE)
	products = models.CharField(max_length=50)
