	...
	if request.user.is_superuser or request.user.is_staff:
		if request.method == 'POST':
			...
			while i <= size_no:
				...
				i+=1
			Product(product_id2=prod_id2,shop=request.user,product_name=prod_name,category=category.objects.get(id=int(cat)),subcategory=subcategory,brand=brand,price=price,price_not=price_not,desc=desc,gst=gst_l,image1=image1).save()
...
def seller_signup(request):
	...
	return render(request, 'saler/seller_signup.html', {'form':form, 'title':'Become a Chef'})
	...
	if request.user.is_superuser or request.user.is_staff:
		if request.method == 'POST':
			...
			if s_form.is_valid() and u_form.is_valid():
				...
				return redirect("saler/account_settings.html")
...
def upload_excel(request):
	...
	if request.user.is_superuser or request.user.is_staff:
		if request.method == 'POST':
			print("Post action")
			excel_file=request.FILES.get("import")
			...
			print("yaha se shuru................................")
			ROW=[]
			for col in range(0, dataframe1.max_row):
				...
				is_Veg=ROW[9]
				is_SugarFree=ROW[10]
				is_HighProtein=ROW[11]
				is_GluttentFree=ROW[12]
				is_Sattvik=ROW[13]
				is_Vegan=ROW[14]
				diet=ROW[15]
				TotalCal=ROW[16]
				specs=ROW[17]
				URL=ROW[18]
				image1 = 'products/images/8125abdq1-card-holder-abys-original-imafynysp4xagnt6.jpeg'
				if Product.objects.all():
					prod_id2 = 'pr'+hex(Product.objects.all().last().product_id+1)
				else:
					prod_id2 = 'pr'+hex(0)
				Product(product_id2=prod_id2, shop=request.user, product_name=prod_name,ingredients=ingredients,TotalTime=TotalTime,
						is_Veg=is_Veg,is_SugarFree=is_SugarFree,is_HighProtein=is_HighProtein,is_GluttentFree=is_GluttentFree,is_Sattvik=is_Sattvik,is_Vegan=is_Vegan,
						diet=diet,TotalCal=TotalCal,specs=specs,URL=URL,
						category=cat, subcategory=subcat,image1=image1).save()
				# p = Product.objects.filter(product_id2=prod_id2)[0]
				#
				# p.save()
				ROW=[]
			# for x in data:
			# 	fields=re.split(r',\s*(?![^\\[|\\]*\))', x)
			# 	print(fields[0])
			# 	print(fields[1])
			# 	print(fields[2])
			# 	print(fields[3])

		return render(request, 'saler/upload_excel.html')
